...
> = {
  ...
  zh: {
    ...
    sections: [
      {
        heading: "",
        body: `<p>欢迎使用Mathiter。本服务条款（"条款"）规定您对Mathiter网站（<a href="https://mathiter.com">mathiter.com</a>）、移动应用程序及所有相关产品和服务（统称"服务"）的访问和使用，这些服务由주식회사 매쓰이터（Mathiter Inc.）（"Mathiter"、"我们"）提供。</p><p>访问或使用本服务即表示您同意受本条款的约束。如果您不同意，请勿使用本服务。</p>`,
      },
      {
        heading: "1. 访问与资格",
        body: `<p>本服务面向年满13周岁的个人开放。18周岁以下的用户必须获得父母或法定监护人的同意。代表未成年人注册的父母或监护人代表该未成年人接受本条款，并对该未成年人使用本服务承担责任。</p><p>我们保留随时修改、暂停或终止服务任何部分的权利，无论是否事先通知。我们也可能验证您的年龄、身份或父母同意，并暂停未通过验证的账户。</p>`,
      },
      {
        heading: "2. 账户与注册",
        body: `<p>要访问某些功能，您可能需要创建账户。您同意在注册时提供准确、最新和完整的信息，并保持账户信息的更新。</p><p>您有责任维护密码的保密性，并对您账户下发生的所有活动负责。您必须在发现任何未经授权使用您账户的情况后立即通知我们。Mathiter不对因您未能保护凭据安全而导致的任何损失承担责任。</p>`,
      },
      {
        heading: "3. 费用与支付",
        body: `<h3>3.1 计费</h3><p>基础服务免费提供。高级功能需要付费订阅。所有费用不含适用税费，税费由您自行承担。您必须保持有效的账单信息。</p><h3>3.2 自动续费</h3><p>付费订阅在每个计费周期结束时自动续费，除非您在续费日期前取消。您可以随时通过账户设置取消。取消后，您将保留对高级功能的访问权限，直到当前计费周期结束。</p><h3>3.3 退款</h3><p>所有付款均为最终付款且不可退还，除非适用法律要求。我们可以自行决定逐案发放退款。</p>`,
      },
      {
        heading: "4. 知识产权",
        body: `<p>服务中的所有内容——包括文本、图形、标志、图像、音频、视频、软件、数学题目、解析和课程材料——均为Mathiter或其许可方所有，受版权、商标和其他知识产权法保护。</p><p>未经我们明确书面许可，您不得复制、修改、分发、出售、许可或基于服务中的任何内容创作衍生作品。</p>`,
      },
      {
        heading: "5. 用户内容",
        body: `<p>您可以向服务提交内容，包括练习题的回答、反馈和其他材料（"用户内容"）。您保留用户内容的所有权，但授予Mathiter全球性、非排他性、免版税的许可，以在运营和改善服务的过程中使用、展示、修改和分发您的用户内容。</p><p>您声明您拥有提交用户内容的所有必要权利，且该内容不侵犯任何第三方权利。</p>`,
      },
      {
        heading: "6. AI功能",
        body: `<p>服务包含AI驱动的功能，如自适应测试、个性化辅导和自动解析（"AI功能"）。您确认：</p><ul><li>AI生成的内容仅供教育目的，可能包含不准确或错误的信息。</li><li>AI输出不能替代专业学术建议或持证教育者的指导。</li><li>我们可能使用您与AI功能互动的去标识化或汇总数据来改善服务。</li><li>我们不会使用您的个人AI互动数据来训练第三方AI模型。</li></ul>`,
      },
      {
        heading: "7. 禁止行为",
        body: `<p>您同意不会：</p><ul><li>使用自动化工具、机器人、爬虫或数据挖掘技术访问或收集服务中的数据。</li><li>试图未经授权访问服务的任何部分、其他用户的账户或我们的系统。</li><li>对服务中使用的任何软件进行逆向工程、反编译或反汇编。</li><li>将服务用于任何非法目的或违反任何适用法律。</li><li>提交虚假、误导性、诽谤性或冒犯性内容。</li><li>干扰服务的正常运行或对我们的基础设施施加不合理的负担。</li><li>未经书面授权使用服务中的内容训练AI或机器学习模型。</li></ul>`,
      },
      {
        heading: "8. 第三方链接",
        body: `<p>服务可能包含指向第三方网站或服务的链接。Mathiter不对任何第三方网站的内容、准确性或做法承担责任。您使用第三方服务的风险由您自行承担，并受其条款和政策约束。</p>`,
      },
      {
        heading: "9. 免责声明",
        body: `<p>服务按"现状"和"可用"的基础提供，不附带任何形式的明示或暗示保证，包括但不限于适销性、特定用途适用性和非侵权保证。我们不保证服务将不中断、无错误或安全，或任何缺陷将被纠正。</p>`,
      },
      {
        heading: "10. 责任限制",
        body: `<p>在法律允许的最大范围内，Mathiter不对因您使用服务而产生的任何间接、附带、特殊、后果性或惩罚性损害承担责任。我们的总责任不超过您在索赔前十二（12）个月内向Mathiter支付的金额或100美元，以较大者为准。</p>`,
      },
      {
        heading: "11. 赔偿",
        body: `<p>您同意就因您使用服务、违反本条款或侵犯任何第三方权利而产生的任何索赔、损害、损失或费用（包括合理的律师费），对Mathiter及其管理人员、董事、员工和代理人进行赔偿并使其免受损害。</p>`,
      },
      {
        heading: "12. 终止",
        body: `<p>我们可以随时暂停或终止您对服务的访问，无论是否有原因或事先通知。终止后，您使用服务的权利立即终止。与知识产权、免责声明、责任限制和赔偿相关的条款在终止后继续有效。</p>`,
      },
      {
        heading: "13. 适用法律",
        body: `<p>本条款受大韩民国法律管辖并据其解释，不考虑法律冲突原则。因本条款产生的任何争议均受大韩民国法院的专属管辖。</p>`,
      },
      {
        heading: "14. 条款变更",
        body: `<p>我们可能不时更新本条款。我们将通过在服务上发布更新后的条款并更新上述"生效日期"来通知您重大变更。变更发布后您继续使用服务即表示您接受更新后的条款。</p>`,
      },
      {
        heading: "15. 联系我们",
        body: `<p>如果您对本条款有任何疑问，请通过以下方式联系我们：</p><p><strong>주식회사 매쓰이터（Mathiter Inc.）</strong><br/>邮箱：<a href="mailto:contact@mathiter.com">contact@mathiter.com</a><br/>网站：<a href="https://mathiter.com">https://mathiter.com</a></p>`,
      },
    ],
  },
};
